...
static void bit_rev_test(void) {
    ...
    CATCH_REQUIRE(REV8(foo8) == 0x80);
    CATCH_REQUIRE(REV16(foo16) == 0x8000);
    CATCH_REQUIRE(REV32(foo32) == 0x80000000);

    CATCH_REQUIRE(REVL8(foo8) == 0x80);
    CATCH_REQUIRE(REVL16(foo16) == 0x8000);
    CATCH_REQUIRE(REVL32(foo32) == 0x80000000);

    foo8 = 0x80;
    foo16 = 0x8000;
    foo32 = 0x80000000;

    CATCH_REQUIRE(REV8(foo8) == 0x1);
    CATCH_REQUIRE(REV16(foo16) == 0x1);
    CATCH_REQUIRE(REV32(foo32) == 0x1);

    CATCH_REQUIRE(REVL8(foo8) == 0x1);
    CATCH_REQUIRE(REVL16(foo16) == 0x1);
    CATCH_REQUIRE(REVL32(foo32) == 0x1);
} /* bit_rev_test() */

static void crc_test(void) {
    uint8_t data[] = { 0x4d,  0x54,  0x30, 0x30, 0x01, 0x02, 0x03, 0x04 };
    uint32_t crc = crc32_eth(data, 8);
    CATCH_REQUIRE(BSWAP32(crc) == 0xb5e0332);

    crc = crc32_ethl(data, 8);
    CATCH_REQUIRE(BSWAP32(crc) == 0x0b5e0332);
} /* crc_test() */
...
static void bswap_test(void) {
    ...
    CATCH_REQUIRE(BSWAP16(foo16) == 0x0100);
    CATCH_REQUIRE(BSWAP32(foo32) == 0x01000000);
    CATCH_REQUIRE(BSWAP64(foo64) == 0x0100000000000000);

    foo16 = 0x8000;
    foo32 = 0x80000000;
    foo64 = 0x8000000000000000;

    CATCH_REQUIRE(BSWAP16(foo16) == 0x80);
    CATCH_REQUIRE(BSWAP32(foo32) == 0x80);
    CATCH_REQUIRE(BSWAP64(foo64) == 0x80);

    foo16 = 0x0123;
    foo32 = 0x01234567;
    foo64 = 0x0123456789ABCDEF;

    CATCH_REQUIRE(BSWAP16(foo16) == 0x2301);
    CATCH_REQUIRE(BSWAP32(foo32) == 0x67452301);
    CATCH_REQUIRE(BSWAP64(foo64) == 0xEFCDAB8967452301);
} /* bswap_test() */
