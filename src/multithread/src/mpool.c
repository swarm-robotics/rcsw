...
struct mpool* mpool_init(struct mpool* const pool_in,
  ...
  CHECK_PTR(llist_init(&the_pool->free, &llist_params));
  llist_params.nodes = params->nodes + llist_node_space(params->max_elts);
  CHECK_PTR(llist_init(&the_pool->alloc, &llist_params));

  size_t i;
  for (i = 0; i < params->max_elts; ++i) {
    CHECK(OK == llist_append(&the_pool->free,
...
status_t mpool_release(struct mpool* const the_pool, uint8_t* const ptr) {
   ...
   * decreased to 0 by a call to ref_remove, so use MAX() so stay
   * non-negative.
   */
  the_pool->refs[index] = MAX(0, the_pool->refs[index] - 1);
...
int mpool_ref_query(struct mpool* const the_pool, const uint8_t* const ptr) {
   ...
   * yet been allocated.
   */
  CHECK(NULL != llist_data_query(&the_pool->alloc, ptr));
