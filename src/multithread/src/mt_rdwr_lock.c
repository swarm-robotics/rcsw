...
status_t mt_rdwr_lock_init(mt_rdwr_lock_t* const rdwr_in, uint32_t flags) {
  ...
  rdwr->n_readers = 0;
  CHECK(NULL != mt_csem_init(&rdwr->order, FALSE, 1, MT_APP_DOMAIN_MEM));
  CHECK(NULL != mt_csem_init(&rdwr->access, FALSE, 1, MT_APP_DOMAIN_MEM));
  CHECK(NULL != mt_csem_init(&rdwr->read, FALSE, 1, MT_APP_DOMAIN_MEM));
...
status_t mt_rdwr_lock_timed_wr_enter(mt_rdwr_lock_t* const rdwr,
  ...
  /* request exclusive access to resource */
  CHECK(OK == mt_csem_timedwait(&rdwr->access, to));
  rval = OK;
