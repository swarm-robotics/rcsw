...
struct rbuffer *rbuffer_init(struct rbuffer *rb_in,
  ...
  FPC_CHECK(NULL, params != NULL, params->tag == DS_RBUFFER,
  ...
  DBGD("type: %s max_elts=%zu el_size=%zu flags=0x%08x\n", RBUFFER_TYPE(rb),
       rb->max_elts, rb->el_size, rb->flags);
  return rb;
...
status_t rbuffer_add(struct rbuffer *const rb, const void *const e) {
  ...
  /* add element */
  ds_elt_copy(rbuffer_data_get(rb, (rb->start + rb->current) % rb->max_elts), e,
              rb->el_size);
...
status_t rbuffer_inject(struct rbuffer *const rb, void (*f)(void *e, void *res),
                        void *result) {
