...
struct darray *darray_init(struct darray *arr_in,
  ...
  FPC_CHECK(NULL, params != NULL, params->tag == DS_DARRAY, params->el_size > 0,
            params->max_elts != 0);
  ...
  DBGD("Capacity=%zu init_size=%zu max_elts=%d el_size=%zu flags=0x%08x\n",
       arr->capacity, params->type.da.init_size, arr->max_elts, arr->el_size,
       arr->flags);
  return arr;
...
status_t darray_insert(struct darray *const arr, const void *const e,
                       size_t index) {
  ...
  if (arr->flags & DS_MAINTAIN_ORDER) {
    ...
    for (i = arr->current; i > index; --i) {
      ds_elt_copy(darray_data_get(arr, i), darray_data_get(arr, i - 1),
                  arr->el_size);
    }      /* for() */
  } else { /* if not, just move element at index to end of array */
    memmove(darray_data_get(arr, arr->current), darray_data_get(arr, index),
            arr->el_size);
  }
...
status_t darray_remove(struct darray *const arr, void *const e, size_t index) {
  ...
  if (arr->flags &
      DS_KEEP_SORTED) { /* shift all items AFTER index down by one */
    memmove(darray_data_get(arr, index), darray_data_get(arr, index + 1),
            (arr->current - 1 - index) * arr->el_size);
  } else { /* overwrite index with last item in array (MUCH faster) */
    memmove(darray_data_get(arr, index), darray_data_get(arr, arr->current - 1),
            arr->el_size);
  }
  arr->current--;
...
status_t darray_index_serve(const struct darray *const arr, void *const e,
                            size_t index) {
  FPC_CHECK(ERROR, arr != NULL, e != NULL, index <= arr->current);
...
int darray_index_query(const struct darray *const arr, const void *const e) {
  ...
  if (arr->sorted) {
    ...
    rval = bsearch_rec(arr->elements, e, arr->cmpe, arr->el_size, 0,
                       (arr->current - 1));
  } else {
    size_t i;
...
status_t darray_data_set(const struct darray *const arr, size_t index,
                         const void *const e) {
...
status_t darray_data_copy(const struct darray *const arr1,
  ...
  FPC_CHECK(ERROR, NULL != arr1, NULL != arr2, arr1->el_size == arr2->el_size,
...
struct darray *darray_filter(struct darray *const arr,
  ...
  for (i = 0; i < arr->current + n_removed; ++i) {
    if (pred(darray_data_get(arr, i - n_removed))) {
      CHECK(darray_insert(farr, darray_data_get(arr, i - n_removed),
                          farr->current) == OK);
  ...
  DBGD("%zu %zu-byte elements filtered out into new list. %zu elements remain "
       "in original list.\n",
       n_removed, arr->el_size, arr->current);
  return farr;
